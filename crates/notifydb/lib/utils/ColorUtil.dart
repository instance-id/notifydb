...
extension GetColor on Color {
  ...
  static Color parse(String text) {
    ...
    if (text.codeUnitAt(0) == '#'.codeUnitAt(0)) {
      if (text.length >= 9) {
        // #ff0000ff
        var value = Color(0xFF000000 | int.parse(text.substring(1, 7), radix: 16));
        return value.withOpacity(int.parse(text.substring(7, 9), radix: 16) / 255);
      ...
      // rgb(255, 0, 0)
      // rgb(100%, 0%, 0%)
      var str = text.substring(4, text.length - 1);
      ...
      // rgba(255, 0, 0, 0.6)
      // rgba(100%, 0%, 0%, 0.6)
      var str = text.substring(5, text.length - 1);
      var colors = str.split(',');
      if (colors.length >= 4) {
        var r = _parseRGB(colors[0]);
        var g = _parseRGB(colors[1]);
        var b = _parseRGB(colors[2]);
        var a = _parseA(colors[3]);
  ...
  static int _parseRGB(String text) {
    text = text.trim();
    int i;
    if (text.endsWith('%')) {
  ...
  static int _parseA(String text) {
    return (255 * double.parse(text.trim())).round().clamp(0, 255);
  }

  static int _parseH(String text) {
    return int.parse(text.trim()) % 360;
  }

  static double _parseSL(String text) {
    text = text.trim();
    if (text.endsWith('%')) {
  ...
  static Color _hslaToColor(int h, double s, double l, int a) {
