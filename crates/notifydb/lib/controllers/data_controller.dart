...
class DataController extends GetxController {
  ...
  // --| Window Position -----------------------------
  var windowDefault =
      '''{"width":1000,"height":550,"is_minimized":false,"is_maximized":false,"is_full_screen":false,"is_active":false}''';
  ...
  void applySettings(Future<AppSettings> appSettings) {
    ...
    appSettings.then((value) => {
          ...
          Logger.write('Window: ${s.windowSize}')
        });
  }
  ...
  Timer runPeriodically(void Function() callback) =>
      Timer.periodic(Duration(seconds: int.parse(autoRefreshInterval)), (timer) => callback());
  ...
  Future<String> markSelected(List<int> selected) async {
    final result = await _dataChannel.invokeMethod('markSelected', selected);
    Logger.debug(result);
    return result;
  }
  ...
  Future<SettingsData> getSettings(String arguments) async {
    final settings = await _settingChannel.invokeMethod('get_app_settings', arguments);
  ...
  Future<AppSettings> getAppSettings(String arguments) async {
    ...
    if ((settings as String).contains('Not_found')) {
      ...
      _app_settings =
          appSettingsFromJson(convert.jsonDecode(settings.substring(1, settings.length - 1))['settings_json']);
    }
